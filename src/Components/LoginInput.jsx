...
function LoginInput() {
    ...
    return (
        <div className="login-box flex flex-col rounded-lg shadow-lg text-white w-full max-w-sm sm:max-w-md md:max-w-lg mx-auto">
            <div className="head bg-[#1a1a1a] p-4 sm:p-5 text-center rounded-t-2xl text-[#FF8A00] text-2xl sm:text-3xl md:text-4xl font-black uppercase" style={{ textShadow: "0 4px 4px rgba(0, 0, 0, 0.25)" }}>
                <h2>Login</h2>
            </div>
                ...
                <form className='p-4 sm:p-6 md:p-8' onSubmit={handleSubmit} method="post">
                    ...
                    <div className="remember-forgot flex justify-between items-center mt-4 sm:mt-5 mb-10 mx-1">
                        <label className='flex items-center cursor-pointer'>
                            <input type="checkbox" name="rememberMe" id="rememberMe" checked={rememberMe} onChange={() => setRememberMe(!rememberMe)} className='appearance-none w-4 h-4 rounded-full border-4 border-[#FF8A00] bg-transparent checked:bg-[#FF8A00] focus:outline-none focus:ring-[#FF8A00] focus:ring-offset-0 cursor-pointer' />
                            <span className='text-xs sm:text-sm ml-1.5 text-[#454545] tracking-tight font-bold pb-0.5'>Remember Me</span>
                        </label>
                        <a href="#!" className='text-[#FF8A00] hover:text-[#fb9d33] text-xs sm:text-sm font-bold tracking-tight transition-all duration-300 hover:underline'>Forgot Your Password?</a>
                    </div>
